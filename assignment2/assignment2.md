...
You are given an undirected graph \(G=(V,E)\) where \(V\) is a list of cities and \(E\) is the road pattern between them. The road \(e=\{u,v\}\) connects city \(u\) and city \(v\), and you know its distance in kilometre \(𝓁(e)\). You want to drive from city \(s\) to city \(t\). You own a small car which can only hold enough gas to cover \(Q\,\mathrm{km}\). There is a gas station in each city, but not between cities. Therefore, you can only take a route if every one of its edges has length at most \(Q\).
...
**Input**: An undirected graph \(G=(V,E)\); edge lengths \(𝓁_e>0\); vertex \(s∈V\).
...
```
Ameet-MST(G,w)
    Soft the edges into a non-increasing order of edge weights w
    T=E
    For each edge e∈E, taken in non-increasing order of weights
        if T-e is a connected graph, then remove e form T
```
